...
import { deleteTodo, getAllTodos } from '../../data/todoThunk'
        ...
        {todos?.map((todo) => (
          <tr className="table-row" key={todo.id}>
            ...
            <td className="table-cell btn_container">
              <button className="btn btn_update" onClick={() => handleUpdate(todo?.id)}>
                Update
              </button>
              <button className="btn btn_delete" onClick={() => handleDelete(todo?.id)}>
                Delete
              </button>
            </td>
          </tr>
