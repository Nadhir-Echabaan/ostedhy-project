...
import { ITodo } from '../../../todos/data/todoTypes'
      ...
      <tbody>
        {data?.data?.map((todo: ITodo) => (
          <tr className="table-row" key={todo.id}>
            ...
            <td className={`table-cell ${todo.completed ? 'status-completed' : 'status-pending'}`}>
              {todo.completed ? 'Completed' : 'Pending'}
            </td>
            <td className="table-cell btn_container">
              <button className="btn btn_update" onClick={() => handleUpdate(todo?.id?.toString())}>
                Update
              </button>
              <button className="btn btn_delete" onClick={() => handleDelete(todo?.id?.toString())}>
                Delete
              </button>
            </td>
          </tr>
